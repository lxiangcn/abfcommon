...
/**
 * 接口访问类，包含所有微信支付API列表的封装， 每个接口有默认超时时间（除提交被扫支付为10s，上报超时时间为1s外，其他均为6s）
 *
 * @author widyhu
 */
class WxPayApi {
	...
	// 异步通知url
	const NOTIFY_URL = "weixin/pay/notify";
	
	// =======【证书路径设置】=====================================
	/**
	 * TODO：设置商户证书路径 证书路径,注意应该填写绝对路径（仅退款、撤销订单时需要，可登录商户平台下载， API证书下载地址：https://pay.weixin.qq.com/index.php/account/api_cert，下载之前需要安装商户操作证书）
	 *
	 * @var path
	 */
	...
	// =======【curl代理设置】===================================
	/**
	 * TODO：这里设置代理机器，只有需要代理的时候才设置，不需要代理，请设置为0.0.0.0和0 本例程通过curl使用HTTP POST方法，此处可修改代理服务器， 默认CURL_PROXY_HOST=0.0.0.0和CURL_PROXY_PORT=0，此时不开启代理（如有需要才设置）
	 *
	 * @var unknown_type
	 */
	...
	// =======【上报信息配置】===================================
	/**
	 * TODO：接口调用上报等级，默认紧错误上报（注意：上报超时间为【1s】，上报无论成败【永不抛出异常】， 不会影响接口调用流程），开启上报之后，方便微信监控请求调用的质量，建议至少 开启错误上报。 上报等级，0.关闭上报; 1.仅错误出错上报; 2.全量上报
	 *
	 * @var int
	 */
	...
	function __construct($options) {
		...
		log_message ( 'debug', "init WxPayApi:" . $this->appid );
	}
	...
	public function unifiedOrder($inputObj, $timeOut = 6) {
		...
		$inputObj->SetMch_id ( $this->merchantid ); // 商户号
		$inputObj->SetSpbill_create_ip ( $_SERVER ['REMOTE_ADDR'] ); // 终端ip $inputObj->SetSpbill_create_ip("1.1.1.1");
	 ...
	 * 支付结果通用通知
	 *
	 * @param function $callback 直接回调函数使用方法: notify(you_function); 回调类成员函数方法:notify(array($this, you_function)); $callback 原型为：function function_name($data){}
	 */
	public function notify($callback, &$msg) {
		...
		log_message ( "debug", "xml notify :" . $xml );
		// 如果返回成功则验证签名
		try {
