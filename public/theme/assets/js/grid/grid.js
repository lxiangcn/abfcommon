    ...
    tbody.find('tr :checkbox').each(function(i, elem) {
        $(elem).click(function(event) {
            ...
            for (var a = 0; a < allBox.length; a++) {
                if (huiber.grid._shift_chk == allBox.eq(a).val()) { place = a;
                    break; }
            }
            //0.2.1未找到shift元素的情况
            if (place == -1) {
...
huiber.grid.makePager = function(pageDiv, cur_page, sumPages, pageMetName) {
    ...
    var fixPageElem = function(start, end, cur) {
        for (var pg = start; pg < end; pg++) {
            if (pg == cur) { linker.append('<span class="current">' + (pg + 1) + '</span>'); } else { linker.append(getPageElem(pg, pg + 1)); }
        }
    };
    ...
    if (sumPages <= fixPages) { //少于需要显示的页面则直接显示
        fixPageElem(0, sumPages, cur_page);
        return;
    }
    //b.大于固定分页数时进行截取操作
    if (cur_page != 0) {
    ...
    ths.each(function(column, elem) {
        ...
        $(elem).click(function() {
            ...
            if ($(sort_el).hasClass("sorting_desc")) {
                direct = 1;
                w_css = ['sorting_desc', 'sorting_asc'];
            }
            ...
            data.sort(function(a, b) { //注意,这里比较的不是对象,而是对象的元素
                var el_a = a[m_keys[column]];
                var el_b = b[m_keys[column]];
