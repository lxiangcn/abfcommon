 ...
 */
class Model_user_auth extends MY_Model {
	...
	function __construct() {
		parent::__construct ();
		$this->load->config ( 'auth/user_auth', TRUE );
		...
		$this->salt_length = $this->config->item ( 'salt_length', 'user_auth' );
		$this->encryption = $this->config->item ( 'encryption', 'user_auth' );
	...
	public function group_check($default_group_id = NULL) {
		...
		$query = $this->db->get_where ( $this->tables ['groups'], array ('id' => $default_group_id ), 1 )->row ();
	...
	public function checkAccess($security, $user_id, $class_name, $method_name) {
		switch ($security) {
			...
			case self::AUTHORITY :
				...
				if (! $this->_getGroup ( $group_id )) { // 检查用户组
	...
	public function update_user($id, array $data, $groups = array()) {
		...
		// 设置用户分组
		if (! $this->config->item ( 'default_group', 'user_auth' ) && empty ( $groups )) {
			$this->set_error ( 'account_creation_missing_default_group' );
			return FALSE;
		}
		// check if the default set in config exists in database
		$query = $this->db->get_where ( $this->tables ['groups'], array ('group_name' => $this->config->item ( 'default_group', 'user_auth' ) ), 1 )->row ();
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
		...
		if ($this->identity_column == 'email' && $this->user_auth_model->email_check ( $email )) {
			$this->set_error ( 'account_creation_duplicate_email' );
			return FALSE;
		} elseif ($this->identity_column == 'username' && $this->user_auth_model->username_check ( $username )) {
			$this->set_error ( 'account_creation_duplicate_username' );
			return FALSE;
		} elseif (! $this->config->item ( 'default_group', 'user_auth' ) && empty ( $groups )) {
			$this->set_error ( 'account_creation_missing_default_group' );
			return FALSE;
		}
		// check if the default set in config exists in database
		...
		// Users table.
		$data = array ('username' => $username,'email' => $email,'created' => time (),'active' => ($manual_activation === false ? 1 : 0) );
	...
	public function logout() {
		...
		if (get_cookie ( $this->config->item ( 'identity_cookie_name', 'user_auth' ) )) {
			delete_cookie ( $this->config->item ( 'identity_cookie_name', 'user_auth' ) );
		}
		if (get_cookie ( $this->config->item ( 'remember_cookie_name', 'user_auth' ) )) {
			delete_cookie ( $this->config->item ( 'remember_cookie_name', 'user_auth' ) );
		}
	...
	public function forgotten_password($identity) {
		...
		$update = array ('lost_password_key' => $key,'lost_password_expire' => time () );
	...
	public function forgotten_password_complete($code, $salt = FALSE) {
		...
		if ($profile) {
			...
			$data = array ('password' => $this->hash_password ( $password, $salt ),'lost_password_key' => NULL,'active' => 1 );
			
			$this->db->update ( $this->tables ['users'], $data, array ('lost_password_key' => $code ) );
	 ...
	 *
	 * @param int $id
	 * @param int $active 状态值 0:带审核；1：可以用；2：禁用
	 */
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($this->db->count_all_results ( $this->tables ['users'] ) > 0) {
			$data = array ('lost_password_key' => NULL,'lost_password_expire' => NULL );
			$this->db->update ( $this->tables ['users'], $data, array ('lost_password_key' => $code ) );
			return TRUE;
		}
		return FALSE;
	...
	public function reset_password($identity, $new) {
		...
		$data = array ('password' => $new,'lost_password_key' => NULL,'lost_password_expire' => NULL );
	...
	public function is_time_locked_out($identity) {
		return $this->is_max_login_attempts_exceeded ( $identity ) && $this->get_last_attempt_time ( $identity ) > time () - $this->config->item ( 'lockout_time', 'user_auth' );
	}
	...
	public function get_last_attempt_time($identity) {
		if ($this->config->item ( 'track_login_attempts', 'user_auth' )) {
			$ip_address = $this->_prepare_ip ( $this->input->ip_address () );
			
			$this->db->select_max ( 'time' );
			if ($this->config->item ( 'track_login_ip_address', 'user_auth' )) $this->db->where ( 'ip_address', $ip_address );
	...
	public function is_max_login_attempts_exceeded($identity) {
		if ($this->config->item ( 'track_login_attempts', 'user_auth' )) {
			$max_attempts = $this->config->item ( 'maximum_login_attempts', 'user_auth' );
			if ($max_attempts > 0) {
	...
	public function set_session($user) {
		$session_data = array ('identity' => $user->{$this->identity_column},'username' => $user->username,'email' => $user->email,'user_id' => $user->id, // everyone likes to overwrite id so we'll use user_id
'old_last_login' => $user->last_login );
	...
	protected function _getNode($class, $method, $status = 1) {
		...
		$where = array ('class' => $class,'method' => $method,'published' => $status );
	...
	protected function _getAccess($group_id, $node_id) {
		...
		$where = array ('group_id' => $group_id,'menu_id' => $node_id );
		$result = $this->db->get_where ( $tableName, $where );
		if ($result->num_rows () > 0) {
			return $result->row ();
		}
