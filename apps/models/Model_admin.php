...
class Model_admin extends MY_Model {
    ...
    public function validate_user($username) {
        $data = array();
        $this->db->select('admin.*,user_groups.name as group_name');
        $this->db->from("admin");
        $this->db->join('user_groups', "user_groups.id = admin.gid");
        $this->db->where('admin.username', $username)->limit(1);
        $query = $this->db->get();
        if ($query->num_rows() == 1) {
    ...
    function get_user_by_id($id = '') {
        ...
        $this->db->from("admin");
        $this->db->join('user_groups', "user_groups.id = admin.gid");
        $this->db->where('admin.id', $id)->limit(1);
        $query = $this->db->get();
        if ($query->num_rows() == 1) {
            $data = $query->row_array();
        }
    ...
    function get_user_by_id_email($email, $id) {
        $data = $this->db->where(array('email' => $email, 'id <>' => $id))->get('admin')->row();
        return $data;
    }
}