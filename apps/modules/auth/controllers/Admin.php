...
class Admin extends Admin_Controller {
    ...
    public function index($page_no = 1) {
        $data_where         = array();
        $order_str          = "id desc";
        $total_rows         = $this->admin->find_count($data_where);
        $pagination_link    = pagination_link("auth/admin/index/", intval($total_rows), 4, $page_no);
        $data['info_list']  = $this->admin->find_all($data_where, '*', $order_str, $pagination_link['offset'], $pagination_link['limit']);
        $data['pagestr']    = $pagination_link['link'];
        $data["page_no"]    = $page_no;
        $data['total_rows'] = $pagination_link['total_page'];

        $this->output("admin_layout", array("body" => "admin/index"), $data);
    }
    ...
    public function edit($id = 0, $page_no = 1) {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() === true) {
                ...
                $this->load->library("auth/admin_auth");
                if ($this->admin_auth->update_user($id, $items, $group_id)) {
                    $this->success("保存数据成功。");
                    redirect(site_url('auth/admin/index/' . $page_no));
                } else {
                    $this->error("保存数据出错。");
    ...
    function add_email_check($email) {
        $result = $this->admin->get_user_by_email($email);
        if ($result) {
    ...
    function email_check($email, $id) {
        $result = $this->admin->get_user_by_id_email($email, $id);
        if ($result) {
    ...
    public function delete($id, $page_no = 1) {
        ...
        if ($obj->id) {
            $this->load->library("auth/admin_auth");
            if ($this->admin_auth->delete_user($id)) {
                $this->success("删除成功。");
                redirect(site_url('auth/admin/index/' . $page_no));
            } else {
                $this->error("删除失败。");
    ...
    public function audit($id = 0, $page_no = 1) {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() === true) {
                ...
                $this->load->library("auth/admin_auth");
                if ($this->admin_auth->activate($id, $active)) {
                    // // 认证审核 推送微信信息
                    // $openid = $obj ['username'];
                    ...
                    $this->error("保存数据出错：" . $this->admin_auth->errors());
                }
            }
        }
        $data['data'] = $obj;
