 ...
 *  abfcommon
 *
 * @package Model_admin_auth
 * @copyright Copyright (c) 2010 - 2016, Orzm.net
 * @license http://opensource.org/licenses/GPL-3.0    GPL-3.0
 * @link http://orzm.net
 * @version 2016-03-21 16:38:17
 * @author Alex Liu<lxiangcn@gmail.com>
 */
class Model_admin_auth extends MY_Model {
    ...
    function __construct() {
        ...
        $this->encryption      = $this->config->item('encryption', 'admin_auth');
    ...
    public function is_admin($id = FALSE) {
        ...
        $admin_group = $this->config->item('admin_group', 'admin_auth');
    ...
    public function update_user($id, array $data, $groups = array()) {
        ...
        // 设置用户分组
        if (!$this->config->item('default_group', 'admin_auth') && empty($groups)) {
            $this->set_error('account_creation_missing_default_group');
            return FALSE;
        }
        // check if the default set in config exists in database
        $query = $this->db->get_where($this->tables['groups'], array('group_name' => $this->config->item('default_group', 'admin_auth')), 1)->row();
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        if ($this->identity_column == 'email' && $this->admin_auth_model->email_check($email)) {
            $this->set_error('account_creation_duplicate_email');
            return FALSE;
        } elseif ($this->identity_column == 'username' && $this->admin_auth_model->username_check($username)) {
            $this->set_error('account_creation_duplicate_username');
            return FALSE;
        } elseif (!$this->config->item('default_group', 'admin_auth') && empty($groups)) {
            $this->set_error('account_creation_missing_default_group');
            return FALSE;
        }
        // check if the default set in config exists in database
    ...
    public function get_last_attempt_time($identity) {
        if ($this->config->item('track_login_attempts', 'admin_auth')) {
            ...
            if ($this->config->item('track_login_ip_address', 'admin_auth')) {
