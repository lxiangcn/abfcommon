...
class Installer {
	...
	public function import_schema() {
		$file = APPPATH . 'assets/dbscript/sitecms.sql';
	...
	public function insert_administrator($username, $password, $email) {
		...
		$password = sha1 ( $this->encryption_key . $password );
		$administrator_sql = "INSERT INTO `" . $this->prefix . "users` (`username`, `password`, `email`, `role_id`, `created`) VALUES ('" . $this->db_escape ( $username ) . "','" . $this->db_escape ( $password ) . "','" . $this->db_escape ( $email ) . "',1," . time () . ")";
		$this->db_query ( $administrator_sql );
	}
