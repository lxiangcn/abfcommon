...
huiber.readlog.readlog = function() {
    ...
    if (date == '') {
        huiber.dialog.showAlert('请先选择要读取日志的日期！');
        return;
    }
    ...
    var callback = function(data) {
        ...
        if (!data['success']) {
            huiber.dialog.showAlert(data['message']);
            return;
        }
        data['errs'].sort(function(a, b) {
            return a['at'] - b['at'] < 0; });
...
huiber.readlog.downlog = function() {
    var date = $.trim($('#f_date').val());
    if (date == '') {
        huiber.dialog.showAlert('请先选择要下载日志的日期！');
        return;
    }
    huiber.readlog.__queryDate = date;
    var params = { date: date };
    ...
    var callback = function(data) {
        data = JSON.parse(data);
        if (data['success']) {
            var downUrl = $('#V_BASE_URL').val() + 'console/console/downlog/' + date;
            //jquery 对象转换到 dom 对象，需要使用get(0)
            $(window).get(0).open(downUrl);
            return;
        } else {
            huiber.dialog.showAlert(data['message']);
            return;
...
huiber.readlog.initHtml = function() {
    ...
    huiber.grid.makeOrderBy(
        $('#logTab'),
        'huiber.readlog.__listDat', ['none', 'at', 'nums', 'time', 'msg'], ['none', 'num', 'num', 'str', 'str'],
        huiber.readlog.toPager
    );
    huiber.grid.makeOrderBy(
        $('#logMonthTab'),
        'huiber.readlog.__logMonthList', ['none', 'month', 'days', 'logs'], ['none', 'str', 'num', 'num'],
        huiber.readlog.toPagerLogMonth
    );
    huiber.grid.makeOrderBy(
        $('#logDayTab'),
        'huiber.readlog.__logDayList', ['none', 'date', 'errs', 'debugs', 'infos', 'warns', 'sqls'], ['none', 'str', 'num', 'num', 'num', 'num', 'num'],
...
huiber.readlog.toViewLog = function(at) {
    ...
    var params = {
        date: huiber.readlog.__queryDate,
        at: at
    };
    var callback = function(data) {
        data = JSON.parse(data);
    ...
    $('#impDayTab').find('td :checkbox').each(function(i, elem) {
        $(elem).click(function(event) {
            ...
            for (var a = 0; a < allBox.length; a++) {
                if (huiber.readlog._shift_chk == allBox.eq(a).val()) { place = a;
                    break; }
            }
            //0.2.1未找到shift元素的情况
            if (place == -1) {
            ...
            var min = Math.min(i, place),
                max = Math.max(i, place);
...
huiber.readlog.importDB = function() {
    ...
    if (dateList.length == 0) {
        huiber.dialog.showAlert('请先指定需要导入的日期项!');
        return;
    }
