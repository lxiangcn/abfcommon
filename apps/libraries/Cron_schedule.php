...
class Cron_schedule {
	...
	public function getPendingSchedules() {
		return $this->CI->db->where ( 'status', self::STATUS_PENDING )->get ( $this->cron_schedule_table_name )->result_array ();
	}
	...
	public function dispatch() {
		...
		foreach ( $schedules as $schedule ) {
			...
			try {
				...
				$result = $this->CI->db->where ( 'schedule_id', $schedule ['schedule_id'] )->where ( 'status', self::STATUS_PENDING )->update ( $this->cron_schedule_table_name, array (
		...
		if ($this->cron_schedule) {
			...
			foreach ( $this->cron_schedule as $jobCode => $jobConfig ) {
				...
				for($time = $now; $time < $timeAhead; $time += 60) {
					...
					$match = $this->matchCronExpression ( $cronExprArr [0], $d ['minutes'] ) && $this->matchCronExpression ( $cronExprArr [1], $d ['hours'] ) && $this->matchCronExpression ( $cronExprArr [2], $d ['mday'] ) && $this->matchCronExpression ( $cronExprArr [3], $d ['mon'] ) && $this->matchCronExpression ( $cronExprArr [4], $d ['wday'] );
	...
	public function cleanup() {
		...
		$history = $this->CI->db->where_in ( 'status', array (
			...
			self::STATUS_MISSED,
			self::STATUS_ERROR 
		) )->get ( $this->cron_schedule_table_name )->result_array ();
		...
		foreach ( $history as $record ) {
			if ((strtotime ( $record ['executed_at'] ) > 0) && (strtotime ( $record ['executed_at'] ) < $now - $historyLifetimes [$record ['status']])) {
	...
	public function updateSchedule($schedule_id, $schedule_data) {
		return $this->CI->db->where ( 'schedule_id', $schedule_id )->update ( $this->cron_schedule_table_name, $schedule_data );
	}

	public function deleteSchedule($schedule_id) {
		return $this->CI->db->where ( 'schedule_id', $schedule_id )->delete ( $this->cron_schedule_table_name );
	}
}
