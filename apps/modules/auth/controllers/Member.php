...
class Member extends Admin_Controller {
	...
	public function index($page_no = 1) {
		$data_where = array();
		$order_str = "id desc";
		$total_rows = $this->users->find_count($data_where);
		$pagination_link = pagination_link("auth/member/index/", intval($total_rows), 4, $page_no);
		$data['info_list'] = $this->users->find_all($data_where, '*', $order_str, $pagination_link['offset'], $pagination_link['limit']);
	...
	public function add($page_no = 1) {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() === true) {
				...
				if ($this->user_auth->register($username, $password, $email, $items, $group)) {
					$this->success('数据保存成功。');
	...
	public function edit($id = 0, $page_no = 1) {
		...
		$data['roles'] = $roles;
		// 查询用户权限
		$user_groups = $this->db->where(array("user_id" => $id))->get('user_groups')->result();
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() === true) {
				...
				$this->load->library("auth/user_auth");
				if ($this->user_auth->update_user($id, $items, $group_id)) {
					$this->success("保存数据成功。");
					redirect(site_url('auth/member/index/' . $page_no));
				} else {
					$this->error("保存数据出错。");
	...
	function email_check($email, $id) {
		$result = $this->users->get_user_by_id_email($email, $id);
		if ($result) {
	...
	public function delete($id, $page_no = 1) {
		...
		if ($obj->id) {
			$this->load->library("auth/user_auth");
			if ($this->user_auth->delete_user($id)) {
				$this->success("删除成功。");
				redirect(site_url('auth/member/index/' . $page_no));
			} else {
				$this->error("删除失败。");
	...
	public function audit($id = 0, $page_no = 1) {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() === true) {
				...
				$this->load->library("auth/user_auth");
				if ($this->user_auth->activate($id, $active)) {
					// // 认证审核 推送微信信息
					// $openid = $obj ['username'];
					...
					$this->error("保存数据出错：" . $this->user_auth->errors());
				}
			}
		}
		$data['data'] = $obj;
